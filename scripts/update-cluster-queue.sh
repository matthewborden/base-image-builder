...
do
  curl -X POST \
    ...
    -H "Content-Type: application/json" \
    -d '{
      "query": "mutation ($organization_id: ID!, $queue_id: ID!, $base_image: String!) { clusterQueueUpdate( input: { organizationId: $organization_id id: $queue_id hostedAgents: { agentImageRef: { $base_image } } } ) { clusterQueue { id hostedAgentSettings { platformSettings { linux { agentImageRef } } } } } }",
